...
// Find line with captcha recordAcceptance
let insertIdx = -1;
for (let i = 0; i < lines.length; i++) {
  const line = lines[i].replace('\r', '');
  if (line.trim() === "const { memberRoleGranted } = await recordAcceptance(") {
    // Check next few lines for captcha
    if (i + 3 < lines.length && lines[i+3].replace('\r','').includes("'captcha'")) {
      insertIdx = i;
      break;
    }
  }
