...
// 1. /thermal - Heat Map
export const thermalCommand: Command = {
    data: new SlashCommandBuilder()
        .setName('thermal')
        .setDescription('Initialize 3D Heat Prism Visualization') as any,
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor(AvenloColors.GREEN) // Emerald for Tactical
...
// 2. /shadow - User Reputation
export const shadowCommand: Command = {
    data: new SlashCommandBuilder()
        .setName('shadow')
        .setDescription('Pull 24h Reputation Sparkline & Shadow Score')
        .addUserOption(opt => opt.setName('user').setDescription('Target user').setRequired(true)) as any,
...
// 3. /forensic - CSI Logic Sheet
export const forensicCommand: Command = {
    data: new SlashCommandBuilder()
        .setName('forensic')
        .setDescription('Open CSI Logic Sheet for a specific Event ID')
        .addStringOption(opt => opt.setName('id').setDescription('Incident ID or Message ID').setRequired(true)) as any,

    async execute(interaction: ChatInputCommandInteraction) {
        const id = interaction.options.getString('id', true);
        // Mock lookup - in real app would query MongoDB
...
// 4. /intercept - User Isolation
export const interceptCommand: Command = {
    data: new SlashCommandBuilder()
        .setName('intercept')
